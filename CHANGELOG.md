...
### Fixed
- Stop locking remember_token by default so the framework's remember-me and logout cycling work for the protected account.

## [0.7.0] - 2026-09-08

### Breaking
- **`ensure()` / `install()` no longer adopt an existing account.** When an ordinary row already holds the target email, provisioning throws `ProtectedAccountException` instead of promoting it. Opt in with `SuperAdmin::ensure([..., 'adopt' => true])` or `php artisan superadmin:ensure --adopt` — and an adoption always rotates the account's password and `remember_token`, so a pipeline that assumed the old password survives must be updated.
- **Writes to `password` / `remember_token` on the protected row now throw** (`superadmin.protection.locked_attributes`) unless they come through `SuperAdmin::withoutProtection()`, `superadmin:ensure`, or the recovery route. Host reset-password actions, bulk updates and impersonation helpers that touch the protected account directly must be routed through a trusted path, or the list set to `[]` in that app.
- **`superadmin:status` prints `configured` / `not configured` only** — never the password value. Deploy checks that grep its output for the credential must grep the state instead.

### Security
- **Role-promotion guard now resolves EVERY role bearing the configured name, matched on id + `guard_name`.** It previously took the first matching id (`where('name', ...)->value('id')`). Spatie legitimately allows the same role name under several guards, and `hasRole('super_admin')` matches the *name* whichever guard issued it — so with an `api` `super_admin` row created first, assigning the `web` `super_admin` row to an ordinary user compared against the wrong id, passed the guard, and still granted the privilege. Every id carrying the name is now forbidden, a payload item that arrives as a Role model is matched on its own name/guard pair (catching a row created after the lookup, or a custom role model), and all matched ids are detached — not just the one. The guard's known limit (post-write compensation; a direct `roles()->attach()`, raw SQL, a suppressed event bus or an earlier listener throwing all bypass it) is now documented on the method rather than implied.
- **Provisioning no longer adopts an existing account implicitly.** `install()` / `ensure()` used to fall back to any row already holding the target email and promote it, keeping that account's password and never rotating its `remember_token` — so an ordinary registration could become the gate-bypassing root identity while its previous owner's credentials and persistent login still worked. That case now throws `ProtectedAccountException` with the address named. Deliberate adoption is opt-in via `SuperAdmin::ensure([..., 'adopt' => true])` or `php artisan superadmin:ensure --adopt`, and an adoption **always** rotates the password (to the supplied one, else a fresh package default) and the remember token.
- **The protected account's credential attributes are now guarded server-side.** `superadmin.protection.locked_attributes` (default `password`, `remember_token`) blocks any Eloquent write to those columns on the protected row outside `SuperAdmin::withoutProtection()`. Disabling the password field in a Filament form is a UI affordance; a host's custom "reset password" action, a bulk update or a direct service write reached the model regardless and could set a password on the god account. Hosts add their own privileged columns (e.g. `status`, `user_type`) to the list, or set it to `[]` to restore the previous behavior. The break-glass recovery route now writes through `withoutProtection()`, as the trusted credential capability.
- **`superadmin:status` no longer prints any password.** The Password row previously echoed the plaintext candidate whenever it verified against the stored hash — including an explicit `SUPER_ADMIN_PASSWORD` in production — and deploy health checks pipe that output into CI logs and log collectors that reach far beyond credential administrators. It now reports state only: "configured — matches SUPER_ADMIN_PASSWORD" / "configured — matches the package default for this environment" / "not configured".

### Fixed
- **Partial `ensure()` updates no longer overwrite the fields you did not pass.** `ensure(['password' => '...'])` supplied `defaultName()` for the omitted name and let `install()` default a null email to `defaultEmail()`, so a password-only call silently renamed the account to "Super Admin" and repointed its email — and therefore its recovery mail — at the derived default address. Omitted keys are now distinguished from explicit nulls: on **update** they leave the field untouched; on **create** they still fall back to the package defaults. `superadmin:ensure --from-env` passes explicit values, so its full reconciliation is unchanged.
- **Recovery session eviction now deletes from the session driver's own connection.** It used the *user model's* connection, so a host keeping users on a tenant/application database while Laravel's database sessions live on a central one (`session.connection`) deleted from the wrong database and left the live session standing after a break-glass reset.
- **`is_protected` column detection is connection-aware and keyed by connection + table.** The check ran on the default connection and memoized one boolean on a container singleton, so a user model on a non-default connection was probed against the wrong schema, and a queue worker or tenant switch reused the previous schema's answer for the rest of the process.
- **Protected-account discovery ignores global scopes.** A host scope (tenant, "approved only") hiding the protected row made `user()` / `exists()` report "missing", which let provisioning install a *second* protected account. This is the same explicit provisioning scope policy `findByEmail()` already applied.
- **`superadmin:status --verbose` reports more than one protected account as a problem** (identity resolution is ambiguous; the lowest id wins) and its password guidance now names the host's *actual* recovery path — CLI only when the web route is disabled, the configured path when it is not, instead of always pointing at `/superadmin`.
...
### Fixed
- **Auto-install no longer echoes an env-sourced `SUPER_ADMIN_PASSWORD` in plaintext to `migrate` output.** Deploy pipelines capture and retain that STDOUT, so a env-configured credential previously landed in CI/deploy log history. When the password comes from `SUPER_ADMIN_PASSWORD`, the migrate output now shows `(password: set via SUPER_ADMIN_PASSWORD)` instead of the literal value; the local/testing literal `superadmin` default is unaffected (never a secret).
